...
bot = telebot.TeleBot('7474580711:AAHzo1nM5aWZEtM37A5rU1gCC6UITYdj0ko')

# Admin user IDs
admin_id = ["1217244173","1217244173"]
...
def show_all_users(message):
    ...
    if user_id in admin_id:
        ...
        response = "Only @Sarug11 Can Run This Command."
    bot.reply_to(message, response)

@bot.message_handler(commands=['owner'])
def show_owner(message):
    response = "👑 Bot Owner: @Sarug11"  # Replace with the actual owner username
    bot.reply_to(message, response)
        ...
        if len(command) > 2:  # Check if the command contains the user ID and time duration
            ...
            try:
                ...
                if user_to_add not in allowed_user_ids:
                    ...
                    save_user_access(user_access)
                    response = f"User {user_to_add} approved for {time_value} {time_unit} by @Sarug11.\n\n\n 🅑🅞🅣 🅛🅘🅝🅚: @Ghokggyih_ddosbot"
                else:
...
def handle_bgmi(message):
    ...
    if user_id in allowed_user_ids:
        ...
        response = ("🚫 Unauthorized Access! 🚫\n\n"
                    ...
                    "👉 Contact an Admin or the Owner @Sarug11 for approval.\n"
                    "🌟 Become a proud supporter and purchase approval.\n"
                    "💬 Chat with an Owner @Sarug11 now and level up your capabilities!\n\n"
...
def clear_logs_handler(message):
    ...
    if user_id in admin_id:
        ...
        bot.reply_to(message, "Only @Sarug11 can run this command.")
